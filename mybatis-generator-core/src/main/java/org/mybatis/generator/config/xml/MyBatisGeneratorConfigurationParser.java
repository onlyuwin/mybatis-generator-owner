...
import com.only.generator.extplugin.config.JavaExampleGeneratorConfiguration;
...
public class MyBatisGeneratorConfigurationParser {
    ...
    public Configuration parseConfiguration(Element rootNode)
        ...
        for (int i = 0; i < nodeList.getLength(); i++) {
            ...
            if ("properties".equals(childNode.getNodeName())) { //$NON-NLS-1$
                ...
                //tip:javaModelGenerator标签和我们新加的javaExampleGenerator都是context的子标签
    ...
    private void parseContext(Configuration configuration, Node node) {
        ...
        for (int i = 0; i < nodeList.getLength(); i++) {
            ...
            if ("property".equals(childNode.getNodeName())) { //$NON-NLS-1$
                ...
                //tip：解析 javaModelGenerator
                parseJavaModelGenerator(context, childNode);
            } else if ("javaExampleGenerator".equals(childNode.getNodeName())) {
                //tip：2018-11-14自定义新增解析 javaExampleGenerator
    ...
    protected void parseTable(Context context, Node node) {
        ...
        //onlyuwin新增逻辑删方法，默认不生成
        String enableRemoveByPrimaryKey = attributes
                .getProperty("enableRemoveByPrimaryKey"); //$NON-NLS-1$
        if (stringHasValue(enableRemoveByPrimaryKey)) {
            tc.setRemoveByPrimaryKeyStatementEnabled(
                    isTrue(enableRemoveByPrimaryKey));
        }
    ...
    protected void parseJavaModelGenerator(Context context, Node node) {
        //tip:1 自定义解析器 JavaModelGeneratorConfiguration
    ...
    private void parseJavaClientGenerator(Context context, Node node) {
        ...
        //onlyuwin新增Mapper继承BaseDao
        String targetSupperClass = attributes.getProperty("targetSupperClass");
        String targetKeyType = attributes.getProperty("targetKeyType");
        ...
        //onlyuwin新增Mapper继承BaseDao
        javaClientGeneratorConfiguration.setTargetSupperClass(targetSupperClass);
        javaClientGeneratorConfiguration.setTargetKeyType(targetKeyType);
