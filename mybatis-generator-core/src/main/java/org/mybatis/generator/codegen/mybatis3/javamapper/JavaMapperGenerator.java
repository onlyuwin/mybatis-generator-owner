...
import com.only.generator.extplugin.xmlmapper.elements.RemoveByPrimaryKeyMethodGenerator;
...
public class JavaMapperGenerator extends AbstractJavaClientGenerator {
    ...
    public List<CompilationUnit> getCompilationUnits() {
        ...
        if (javaClientGeneratorConfiguration != null && javaClientGeneratorConfiguration.getTargetSupperClass() != null
                && !"null".equals(javaClientGeneratorConfiguration.getTargetSupperClass())) {
            String entity = introspectedTable.getBaseRecordType();
            String entityExample = introspectedTable.getExampleType();
            String superInterface = javaClientGeneratorConfiguration.getTargetSupperClass()
                    + "<" + javaClientGeneratorConfiguration.getTargetKeyType()
                    + "," + entity
                    + "," + entityExample +
                    ">";
            FullyQualifiedJavaType fqjt = new FullyQualifiedJavaType(superInterface);
            interfaze.addSuperInterface(fqjt);

            FullyQualifiedJavaType fqjType = new FullyQualifiedJavaType("org.springframework.stereotype.Repository");
            interfaze.addImportedType(fqjType);
            ...
            //onlyuwin新增逻辑删除方法默认不生成
            addRemoveByPrimaryKeyMethod(interfaze);
        }
    ...
    //onlyuwin新增逻辑删除方法，默认不生成
    protected void addRemoveByPrimaryKeyMethod(Interface interfaze) {
        if (introspectedTable.getRules().generateRemoveByPrimaryKey()) {
            AbstractJavaMapperMethodGenerator methodGenerator = new RemoveByPrimaryKeyMethodGenerator(false);
            initializeAndExecuteGenerator(methodGenerator, interfaze);
        }
    }
